...
OUT=Path(__file__).resolve().parent/'shell-v3'
...
def interp(z,table):
    for (a,x),(b,y) in zip(table,table[1:]):
        if z<=b:return x+(y-x)*max(0,(z-a)/(b-a))
    return table[-1][1]
...
for j in range(NZ+1):
    ...
    for i in range(NX+1):
        u=-1+2*i/NX;au=abs(u)
        x=u*(.51+(.79-.51)*t)
        top=1.21+.23*au+.30*au**12
        bottom=.04+.34*au
        z=bottom*(1-t)+top*t
        y=(-.78+.33*au**1.3)*(1-t)+(-.98+1.10*(1-math.sqrt(max(0,1-au*au))))*t
        v.append((x,y,z))
f=[]
for j in range(NZ):
    for i in range(NX):
...
for s in [-1,1]:
    ...
    for j in range(NZ+1):
        ...
        for i in range(NX+1):
            ...
            y=(-.43+.58*q)*(1-t)+(.08+.12*q)*t-.15*math.sin(t*math.pi)*(1-q)
...
bpy.ops.export_scene.gltf(filepath=str(OUT/'Numen_1_helmet_shell_v3.glb'),use_selection=True,export_apply=True)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'Numen_1_helmet_shell_v3.blend'))
